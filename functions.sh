...
# $5: max db days, $6: volatile database?
function install_pihole() {
  ...
  cp ./pihole/Makefile "${TARGET_DIRECTORY}/"
  sed \
  ...
  local CONFIG_DIRECTORY="${TARGET_DIRECTORY}/data/pihole"
  local FTL_CONF="${CONFIG_DIRECTORY}/pihole-FTL.conf"
  mkdir -p "$CONFIG_DIRECTORY"
  echo "MAXDBDAYS=${5}" > "$FTL_CONF"
  if [ "${6}" = "yes" ]; then
    echo "DBFILE=/tmp/pihole-FTL.db" >> "$FTL_CONF"
  fi
